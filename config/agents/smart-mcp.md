...
3. **執行 todo 項目的原則**
    ...
    - 執行前先確認理解該項目，不清楚時優先使用 `smart_smart_run({tool:"exa_search", args:{command:"search", query:"..."}})` 上網找答案（比 `websearch` 更結構化、支援三種模式、可加 `clean:true` `markdown:true` 萃取乾淨文章），沒有合適結果才退而使用 `websearch`
   - 完成每一個任務後立即更新 todo 狀態，再處理下一個 todo
   - 同一個 todo 使用工具超過兩次失敗時：
     - 發動 `smart_smart_think` 重新分析
      - 使用 `smart_smart_run({tool:"exa_search"})`（search / crawl / code 三模式）或 `websearch` 上網尋找解決方法
...
| **網路搜尋** | `websearch` | `smart_smart_run` + `exa_search` | **3 種模式 + 4 項強化，全面取代 websearch**：`search`（一般搜尋）→ `smart_smart_run({ tool: "exa_search", args: { command: "search", query: "...", numResults: 10 } })`；`crawl`（爬取頁面）→ `smart_smart_run({ tool: "exa_search", args: { command: "crawl", urls: ["..."], clean: true, markdown: true } })` 可加入 `clean: true` 萃取文章、`markdown: true` 轉 MD、`fetchOnly: true` 免 API key、`noCache: true` 跳過快取；`code`（程式碼搜尋）→ `smart_smart_run({ tool: "exa_search", args: { command: "code", query: "...", numResults: 5 } })` |
...
websearch           smart_smart_run → exa_search           ← 3 模式(search/crawl/code) + 4 強化(clean/markdown/fetchOnly/noCache)，全面取代 websearch
...
| **網路研究** | `smart_smart_run({ tool: "exa_search", args: { command, query, ... } })` | **3 模式 + 4 強化**：`search`（一般搜尋，附日期/作者/Highlights）、`crawl`（爬取全文，可加 `clean:true` 萃取文章、`markdown:true` 轉 MD、`fetchOnly:true` 免 API key）、`code`（GitHub 程式碼搜尋，含 README + 程式碼片段） |
...
### 🚀 exa_search 完全取代 websearch

`exa_search` 是 Smart MCP 提供的專業網路研究工具，透過 **3 種確定性模式 + 4 項強化功能** 全面取代內建 `websearch`。結果更結構化、支援爬取全文、文章萃取、Markdown 轉換、快取與離線備援。

#### 為什麼優於 websearch？

| 比較維度 | `websearch`（內建） | `exa_search`（Smart MCP） |
|---------|-------------------|--------------------------|
| **輸出格式** | 純文字摘要，缺乏結構 | 結構化 JSON：標題/URL/日期/作者/Highlights |
| **搜尋模式** | 僅一般搜尋 | 3 種：search / crawl / code |
| **程式碼搜尋** | ❌ 不支援 | ✅ GitHub 程式碼直接搜尋（含 README + 程式碼片段） |
| **頁面爬取** | ❌ 不支援 | ✅ 指定 URL 爬取全文，控制 maxChars |
| **文章萃取** | ❌ 不支援 | ✅ `--clean` 使用 Mozilla Readability 移除 nav/ads/footer，只留文章主體 |
| **Markdown 輸出** | ❌ 不支援 | ✅ `--markdown` Turndown 轉為 LLM-friendly Markdown（headings/code/links 保留） |
| **離線備援** | ❌ 不可用 | ✅ `--fetch-only` 純 HTTP fetch，零 API key，完全離線可運作 |
| **快取** | ❌ 不支援 | ✅ SQLite 自動快取，相同請求秒回，TTL 5 分鐘，LRU 淘汰 |
| **結果數量** | 固定 | ✅ 自訂 `numResults`（1-20） |
| **發布日期** | ❌ 多數無日期 | ✅ 有日期欄位，可判斷資訊時效 |

#### 三種模式詳解

**① `command: "search"` — 一般網路搜尋**

回傳結構化結果陣列，每筆含 `title`、`url`、`publishedDate`、`author`、`highlights`。

```
smart_smart_run({tool:"exa_search", args:{
  command: "search",
  query: "OpenAI GPT-5 capabilities 2026",
  numResults: 10        // 可選，預設 10
}})
```

**② `command: "crawl"` — 爬取指定 URL 全文（含 4 項強化選項）**

取得指定網頁的完整內容，適合深入閱讀文章、文件、公告。

```
// 基本爬取
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://openai.com/index/introducing-gpt-5/"],
  maxChars: 5000        // 可選，限制回傳字數
}})

// ✅ 進階：Readability 文章萃取（移除 nav/ads/footer，節省 40-60% token）
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://example.com/article"],
  clean: true           // ← 使用 Mozilla Readability 萃取文章主體
}})

// ✅ 進階：Markdown 輸出（LLM-friendly 格式）
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://example.com/article"],
  markdown: true        // ← Turndown 轉為 Markdown
}})

// ✅ 最佳組合：Readability + Markdown（乾淨文章直接餵 LLM）
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://nodejs.org/en/about"],
  clean: true,          // ← 先萃取文章
  markdown: true        // ← 再轉 Markdown（heading/code block/link 保留）
}})
// 輸出範例：
...
// ✅ 離線備援：純 HTTP fetch，零 API key
// 當 EXA_API_KEY 未設定時，crawl 會自動降級到原生 fetch
// 也可強制使用 fetch：
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://example.com"],
  fetchOnly: true,       // ← 跳過 Exa API，直接 HTTP fetch（無 API key 也 OK）
  clean: true,
  markdown: true
}})
...
smart_smart_run({tool:"exa_search", args:{
  command: "crawl",
  urls: ["https://example.com"],
  noCache: true         // ← 跳過 SQLite 快取
}})
```

> **自動快取**：所有 crawl 結果會自動快取在 `~/.smart/cache/cache.db`（SQLite），相同 URL + 選項的請求在 TTL（5 分鐘）內是 **秒回** 的。不需要手動管理。

**③ `command: "code"` — 程式碼搜尋（GitHub）**

搜尋 GitHub 上的真實程式碼範例，回傳 Repository 名稱、README、文件路徑與程式碼片段。適合找實作參考、API 使用模式。

```
smart_smart_run({tool:"exa_search", args:{
  command: "code",
  query: "MCP server implementation TypeScript",
  numResults: 5         // 可選，預設 10
}})
```

#### 四項強化功能快速總覽

| 選項 | 用途 | 技術 | 是否需要 API key |
|------|------|------|-----------------|
| `clean: true` | 文章萃取，移除 nav/ads/footer | Mozilla Readability | ❌ 不需要 |
| `markdown: true` | HTML → Markdown 轉換 | Turndown | ❌ 不需要 |
| `fetchOnly: true` | 強制 HTTP fetch，跳過 Exa API | Node.js 原生 fetch | ❌ 不需要 |
| `noCache: true` | 跳過快取（強制重新爬取） | SQLite | ❌ 不需要 |

**最佳組合**：`clean: true` + `markdown: true` = Readability 萃取文章 → Turndown 轉 Markdown = 乾淨、結構化、LLM-ready 的內容，**完全不需要 API key**。

#### 使用原則

1. **一般查詢** → 優先使用 `exa_search` 的 `search` 模式（結構化結果，方便比對時效性與來源）
2. **需要全文** → 用 `search` 找到相關頁面後，再用 `crawl` 爬取完整內容
3. **需要乾淨文章餵 LLM** → `crawl` + `clean: true` + `markdown: true`（最佳組合）
4. **無 API key 時** → crawl 會自動降級到 HTTP fetch，也可自訂 `fetchOnly: true`
5. **開發迭代、重複查同一 URL** → 自動快取，無需手動管理
6. **找程式碼範例** → 直接用 `code` 模式搜尋 GitHub
7. **退路** → 只有 `exa_search` 無法滿足需求時（如需要特定搜索引擎的結果），才用內建 `websearch`

#### 常見工作流

```
研究新技術 — 有 API key:
  smart_smart_run({tool:"exa_search", args:{command:"search", query:"技術關鍵字 2026", numResults:5}})
  → 選定最相關的 URL
  → smart_smart_run({tool:"exa_search", args:{command:"crawl", urls:["選定URL"], clean:true, markdown:true}})
  → smart_smart_thinking({template:"research", topic:"分析結果"})

研究新技術 — 無 API key（完全離線）:
  smart_smart_run({tool:"exa_search", args:{command:"crawl", urls:["https://example.com/article"],
    fetchOnly:true, clean:true, markdown:true}})
  → 純 HTTP fetch + Readability 萃取 + Markdown 轉換
  → 零 API key，零外部服務

深入閱讀多篇文章:
  smart_smart_run({tool:"exa_search", args:{command:"crawl", urls:["url1","url2","url3"],
    clean:true, markdown:true}})
  → 每篇都自動萃取文章 + 轉 Markdown
  → 結果自動快取，下次秒回

找實作參考:
  smart_smart_run({tool:"exa_search", args:{command:"code", query:"特定套件 usage example", numResults:3}})
  → smart_smart_run({tool:"github_search", args:{query:"特定套件", language:"ts"}})

使用快取管理（Debug）:
  smart_smart_run({tool:"crawl", args:{command:"crawl", ...}})     ← 第一次爬
  smart_smart_run({tool:"crawl", args:{command:"crawl", ...}})     ← 第二次 <1ms (cached!)
  smart_smart_run({tool:"crawl", args:{command:"crawl", ..., noCache:true}})  ← 強制重爬
```
...
研究調查（三階段：搜尋 → 爬取 + 清理 → 分析）:
  smart_smart_run({tool:"exa_search", args:{command:"search", query:"...", numResults:5}})
  → smart_smart_run({tool:"exa_search", args:{command:"crawl", urls:["<最佳結果URL>"], clean:true, markdown:true}})
  → 爬取+清理一步完成：Readability 萃取文章 + Turndown 轉 Markdown
  → smart_smart_thinking({template:"research", topic:"..."})
  → smart_smart_run({tool:"report", args:{type:"research", title:"..."}})

離線研究（零 API key）:
  smart_smart_run({tool:"exa_search", args:{command:"crawl", urls:["<URL>"], fetchOnly:true, clean:true, markdown:true}})
  → 純 Node.js fetch + Readability 萃取 + Markdown 轉換
  → 不需任何 API key，完全離線可運作
