...
| 目標追蹤 | `ssr(goal command:"set", description:"...")` → **自動**建立 todo + 每步後自檢查條件 → 達標後 `ssr(goal command:"check", checkResult:"met")` → goal+todo 自動完成 → 匯報 |
...
2️⃣ 自動行為（LLM 自主遵守）
   ...
   - 條件滿足 → ssr({tool:"goal", args:{command:"check", checkResult:"met", checkSummary:"..."}})
     → goal 自動完成 + todo 自動完成 → 向使用者回報 ✅（不需再 call clear）
   - 條件不滿足 → 繼續下一步，無需問使用者
