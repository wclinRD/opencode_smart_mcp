...
| **修改檔案** | `edit` | `smart_smart_run` + `edit.cross_file_edit` | `smart_smart_run({ tool: "edit.cross_file_edit", args: { file, pattern, replacement } })` |
| **批次改多檔** | 多次 `edit` | `smart_smart_run` + `edit.cross_file_edit` | 同上，支援多檔案 atomic |
...
edit   (手動)       smart_smart_run → edit.cross_file_edit ← 更安全，atomic
bash test           smart_smart_test (auto-detect)        ← 不用記測試框架參數
