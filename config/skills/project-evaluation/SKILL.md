...
Step 1: 從 Phase 1 取得專案定位與關鍵功能摘要
  找出「這個專案解決什麼問題」「核心功能是什麼」
  範例：專案是「MCP server 提供開發者工具 + 洋蔥架構 agent」
  核心功能：程式碼分析 / 編輯 / 搜尋 / 爬蟲
...
Step 2b（Layer B—產品層）: 【🟥 強制】搜尋同類競品與替代方案
  ...
  smart_exa_search({
    command:"search",
    query:"{專案類型} alternatives comparison 2026",
    numResults:5,
    compress:"caveman",
    compressLevel:"semantic"
  })
  ...
  範例查詢（依專案類型調整）：
  - MCP server → "MCP server developer tools comparison 2026"
  - AI coding tools → "AI coding assistant open source alternatives"
  - npm package → "{packageName} alternatives npm 2026"
  - Web service → "{service type} competitors comparison"

  對每個找到的競品至少打開 1 篇深入比較：
  smart_exa_crawl({urls:"...", clean:true, markdown:true})
