...
Step 1: 從 Phase 1 取得專案定位與關鍵功能摘要
  找出「這個專案解決什麼問題」「核心功能是什麼」「屬於哪種產品類別」
  然後使用類別對照表決定 Layer B 搜尋方向。

  產品類別判斷對照表（依 Phase 1 結果匹配最接近的類別）：

  | 專案特徵 | 🏷️ 產品類別 | Layer B 搜尋策略 | 範例專案 |
  |----------|-------------|-----------------|---------|
  | AI agent + 工具集、LLM 程式碼生成 | **AI 程式碼助手** | 找 Claude Code、Cursor、Codex 等競品比較 | opencode+Smart MCP |
  | 純 MCP server、無 agent | **MCP Server** | 找同類 MCP server 比較 | 官方 filesystem MCP |
  | npm 套件庫、library | **Library / Package** | 找 npm alternatives | lodash, axios |
  | Web UI + API、SaaS | **Web 應用** | 找同類 SaaS 比較 | Notion, Linear |
  | CLI 工具、cmd line | **CLI 工具** | 找同類 CLI 比較 | jq, fzf |
  | 框架、platform | **Framework** | 找同類框架比較 | React, Vue |
  | VS Code extension | **IDE 擴充** | 找同類 extension 比較 | GitLens, Prettier |

  ⛔ 常見錯誤：只從技術實作面（MCP server）找競品，忽略了**產品面向**（AI coding assistant）的競品
  🟥 強制規則：product category 決定搜尋方向，不得隨便套用範例
...
Step 2b（Layer B—產品層）: 【🟥 強制】搜尋同類競品與替代方案
  ...
  先決定 product category，再使用對應的搜尋策略：

  🏷️ AI 程式碼助手 → 搜尋策略：
    查詢競品總覽：
    smart_exa_search({
      command:"search",
      query:"Claude Code vs Cursor vs Codex vs Cline AI coding comparison 2026",
      numResults:8,
      compress:"caveman",
      compressLevel:"semantic"
    })
    各競品深度：
    smart_exa_crawl({urls:"{競品官網或比較文章}", clean:true, markdown:true})

  🏷️ MCP Server → 搜尋策略：
    smart_exa_search({
      command:"search",
      query:"MCP server comparison best tools 2026",
      numResults:5,
      compress:"caveman",
      compressLevel:"semantic"
    })

  🏷️ Library / Package → 搜尋策略：
    smart_exa_search({
      command:"search",
      query:"{package name} alternatives npm 2026",
      numResults:5,
      compress:"caveman",
      compressLevel:"semantic"
    })
  ...
  🏷️ Web 應用 → 搜尋策略：
    smart_exa_search({
      command:"search",
      query:"{service type} competitors alternatives comparison 2026",
      numResults:5,
      compress:"caveman",
      compressLevel:"semantic"
    })

  對每個找到的競品至少打開 1 篇深入比較：
