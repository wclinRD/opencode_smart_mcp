...
Step 2a（Layer A—依賴層）: 【🟥 強制】對每個依賴搜尋生態健康度
  ...
  提示：{ecosystem} 應從 Phase 1 取得的技術棧推斷（npm/pip/cargo/gem/maven 等）

  smart_exa_search({
    command:"search",
    query:"{tech} {ecosystem} latest version",
    numResults:3,
    compress:"caveman",
...
Step 2b（Layer B—產品層）: 【🟥 強制】搜尋同類競品與替代方案
  ...
  smart_exa_search({
    command:"search",
    query:"{Step 1 產品描述} alternatives comparison {currentYear}",
    numResults:8,
  ...
  smart_exa_search({
    command:"search",
    query:"{這個專案解決的問題} best tools alternatives {currentYear}",
    numResults:8,
    compress:"caveman", compressLevel:"semantic"
  })
  ...
  smart_exa_search({
    command:"search",
    query:"{競品A} vs {競品B} vs {競品C} comparison {currentYear}",
    numResults:5,
    compress:"caveman", compressLevel:"semantic"
  })
