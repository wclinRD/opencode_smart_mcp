...
Devtool MCP 是一個本地開發工具伺服器，透過 MCP 協定為 opencode agent 提供 44 個開發工具 + 專屬 agent personality。當前版本 3.7.2（Plugin Loader + Router 架構 + 動態多輪推理 + Context 管理 + Workflow 引擎 + Compose 引擎 + LSP 程式碼語義分析 + CKG 程式碼知識圖譜 + Hybrid Reasoning Engine + Change-Impact Pipeline + Patch Generation + Agent Personality v2 + 全面非阻塞 CLI + auto-toonify 輸出攔截器 + 多語言支援 (Rust/Swift) + Phase 9 核心元件測試 34/34 通過）。
...
| 任務 | 詳細 |
|------|------|
| C.1 重構助手 | API 使用分析 + 遷移計畫生成 + 安全閘門 |
| C.2 測試預測 | CKG 記錄函式→測試映射 + 增量執行 |
| C.3 健康儀表板 | CKG 統計 + 技術債指數 + 循環依賴可視化 |
| Phase H | 文檔補完 + npm publish smart-agent |
