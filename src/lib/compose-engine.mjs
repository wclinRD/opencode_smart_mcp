...
const TOOL_ARGS_CONVERTERS = {
  smart_grep: (a) => { const c = []; if (a.pattern) c.push(String(a.pattern)); if (a.root) c.push('--root', String(a.root)); if (a.include) c.push('--include', String(a.include)); if (a.exclude) c.push('--exclude', String(a.exclude)); if (a.context) c.push('--context', String(a.context)); if (a.withScope) c.push('--with-scope'); if (a.withImports) c.push('--with-imports'); if (a.ignoreCase) c.push('--ignore-case'); if (a.filesOnly) c.push('--files-only'); if (a.maxMatches) c.push('--max-matches', String(a.maxMatches)); if (a.format) c.push('--format', String(a.format)); if (a.semantic) c.push('--semantic'); if (a.semanticWeight != null) c.push('--semantic-weight', String(a.semanticWeight)); c.push('--no-color'); return c; },
  ...
  smart_exa_search: (a) => { const c = []; if (a.command) c.push(String(a.command)); if (a.command === 'crawl') { if (a.urls) { const urls = String(a.urls).split(',').map(u => u.trim()).filter(Boolean); c.push(...urls); } } else if (a.query) c.push(String(a.query)); if (a.numResults) c.push('--num-results', String(a.numResults)); if (a.maxChars) c.push('--max-chars', String(a.maxChars)); if (a.format) c.push('--format', String(a.format)); if (a.searchType) c.push('--search-type', String(a.searchType)); if (a.category) c.push('--category', String(a.category)); if (a.highlights) c.push('--highlights'); if (a.includeDomains) c.push('--include-domains', JSON.stringify(a.includeDomains)); if (a.excludeDomains) c.push('--exclude-domains', JSON.stringify(a.excludeDomains)); if (a.startDate) c.push('--start-date', String(a.startDate)); if (a.endDate) c.push('--end-date', String(a.endDate)); c.push('--no-color'); return c; },
