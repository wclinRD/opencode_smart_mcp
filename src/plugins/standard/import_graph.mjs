...
const DEFAULT_INDEX_DB = resolve(os.homedir(), '.smart', 'codebase-index.db');

export default {
  name: 'smart_import_graph',
  category: 'analyze',
  description: 'Use when: need to understand cross-file import dependencies before refactoring. Supports JS/TS, Python, Ruby, Rust, Go. Configure depth for shallow or deep analysis. Use focus to zoom in on specific files. Auto-uses codebase index if available (5-50x faster).',
  ...
  mapArgs(a) {
    ...
    if (a.depth) cli.push('--depth', String(a.depth));
    if (a.focus) cli.push('--focus', String(a.focus));
    if (a.noExternals) cli.push('--no-externals');
    if (a.reverse) cli.push('--reverse');
    if (a.format) cli.push('--format', String(a.format));
    // Auto-use codebase index if present (5-50x faster)
    if (existsSync(DEFAULT_INDEX_DB)) cli.push('--use-index');
    cli.push('--no-color');
    return cli;
  },
};
